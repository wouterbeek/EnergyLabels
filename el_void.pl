...
assert_el_void(G, Dir):-
  ...
  % void:documents
  rdf_assert_integer(DD, void:documents, xsd:integer, 10, G),
...
assert_foaf_agentschapnl(G):-
  rdf_global_id(el:'AgentSchapNL', S),
  % foaf:homepage
  rdf_assert(S, foaf:homepage, 'http://www.agentschapnl.nl', G),
  % rdf:type
  rdf_assert_individual(S, foaf:'Organization', G),
  % rdfs:label
  rdfs_assert_label(S, nl, 'Agentschap NL', G).

assert_foaf_wouterbeek(G):-
  rdf_global_id(el:'WouterBeek', S),
  % foaf:mbox
  rdf_assert(S, foaf:mbox, 'mailto:me@wouterbeek.com', G),
  % rdf:type
  rdf_assert_individual(S, foaf:'Person', G),
  % rdfs:label
  rdfs_assert_label(S, nl, 'Wouter Beek', G).
